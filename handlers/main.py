...
def post_inline_keyboard(user_id: int, message_id: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[
            ...
            [InlineKeyboardButton(text="🤝 Поддержать", callback_data=f"support:{user_id}")]
        ]
    )
...
async def ensure_profile_complete(user: User, reply_fn):
...
async def cmd_win(msg: Message, state: FSMContext):
    async with async_session() as ses:
        ...
        if not await ensure_profile_complete(user, msg.answer): return
...
async def handle_sos_text(msg: Message, state: FSMContext):
    ...
    async with async_session() as ses:
        ...
        full_text = (
            f"{text}\n\n"
            f"—\n"
            f"{user.avatar_emoji} {user.pseudo}  | {sobriety}  | 0 ответов"
        )
        ...
        await sent.edit_reply_markup(reply_markup=post_inline_keyboard(user.id, sent.message_id))

        ses.add(Post(id=sent.message_id, author_id=user.id, thread_id=TOPICS["sos"], text=text))
        await ses.commit()
...
async def handle_win_text(msg: Message, state: FSMContext):
    ...
    async with async_session() as ses:
        ...
        ses.add(Post(id=sent.message_id, author_id=user.id, thread_id=TOPICS["wins"], text=text))
        await ses.commit()
